function [commMatrix] = generateCommunicationTopology(coords, A_ij, K)
    ...
    for i = 1:numOfDGs
        for j = 1:numOfDGs  % Check for diagonal and off-diagonal links
            % Check if the maximum absolute value of the controller gain is greater than epsilon
            maxNorm = max(abs(K{i, j}(:)));  % Find the maximum absolute value in the block matrix
